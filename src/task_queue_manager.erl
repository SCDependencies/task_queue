...
init(Options) ->
  {ok, #state{
    ...
    max_length   = proplists:get_value(max_length, Options, undefined)
  }}.
...
handle_cast({Method, Task}, State)
  ...
  NewState =
    case there_are_waiting_workers(State) of
      ...
      false ->
        case is_overflown(State) of
          true  -> State;
          false -> enqueue_task(Method, Task, State)
        end
    end,
  {noreply, NewState};
...
is_overflown(#state{max_length = undefined}) ->
  false;
is_overflown(#state{queue_length = QueueLength, max_length = MaxLength}) ->
